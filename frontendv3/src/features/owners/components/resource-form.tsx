...
export function ResourceForm({ item, onClose, open }: Props) {
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      if (isEdit && item?.id) {
        await updateMutation.mutateAsync({ id: item.id, data: payload as unknown as OwnerUpdate })
      } else {
        await createMutation.mutateAsync(payload as unknown as OwnerCreate)
      }
      onClose()
    } catch (_e) {
      // _e is caught error
    ...
    <Sheet open={open} onOpenChange={(v) => { if (!v) onClose() }}>
      <SheetContent className="flex flex-col">
            ...
            <>
              <FormField control={form.control} name="name" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Input {...field} value={field.value ?? ''} data-testid="owner-name-input" /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
              <FormField control={form.control} name="email" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Input {...field} value={field.value ?? ''} data-testid="owner-email-input" /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
              <FormField control={form.control} name="phone" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Input {...field} value={field.value ?? ''} data-testid="owner-phone-input" /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
            </>
        ...
        <SheetFooter>
          ...
          <Button type="submit" form="owners-form" disabled={loading} data-testid="owner-submit-button">

...
export default function ProjectPage() {
  ...
  const canCreate = useCan('projects', 'add')
  const canEdit = useCan('projects', 'edit')
  const canDelete = useCan('projects', 'delete')

  const { data, isPending, isError, refetch } = useProjects(page, pageSize)
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
         ...
         {canCreate && <Button onClick={() => { setEditing(null); setOpen(true) }} data-testid="add-project-button">Add Project</Button>}
      </div>
      ...
      <ResourceDeleteDialog
        ...
        deleteFn={(id: string) => api.delete('/projects/' + id).then(r => r.data)}
        queryKey={['projects']}
      />
    </div>
  )
